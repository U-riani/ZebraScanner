...
namespace ZebraSCannerTest1.Core.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<IEnumerable<LootBarcodeProgress>> GetLootBarcodeProgressByBoxAsync(string boxId, int limit = 8)
        {
            try
            {
                return await _lootsRepo.GetBarcodeProgressByBoxAsync(boxId, limit);
            }
            catch (Exception ex)
            {
        ...
        public async Task<LootBarcodeProgress?> GetLootBarcodeProgressAsync(string barcode, string boxId)
        {
            try
            {
                return await _lootsRepo.GetBarcodeProgressAsync(barcode, boxId);
            }
            catch (Exception ex)
            {
                _logger.Error("Failed to load loot barcode progress", ex);
                return null;
            }
        }
