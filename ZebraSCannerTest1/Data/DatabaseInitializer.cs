...
namespace ZebraSCannerTest1.Data
{
    public static class DatabaseInitializer
    {
            ...
            string? assignmentRole = null)
        {
            ...
            lock (InitializeLock)
            {
                if (!InitializedDatabasePaths.Contains(dbPath))
                {
                    Initialize(
                        conn,
                        userId,
...
CREATE INDEX IF NOT EXISTS idx_products_scanned_updated
ON Products(ScannedQuantity, UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_loots_products_barcode
ON LootsProducts(Barcode);

CREATE INDEX IF NOT EXISTS idx_loots_products_box
ON LootsProducts(Box_Id);

CREATE INDEX IF NOT EXISTS idx_loots_products_barcode_box
ON LootsProducts(Barcode, Box_Id);

CREATE INDEX IF NOT EXISTS idx_loots_products_box_updated
ON LootsProducts(Box_Id, UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_loots_products_scanned_box
ON LootsProducts(ScannedQuantity, Box_Id);

CREATE INDEX IF NOT EXISTS idx_scan_logs_barcode_updated
ON ScanLogs(Barcode, UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_scan_logs_updated
ON ScanLogs(UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_loots_scan_logs_barcode_updated
ON LootsScanLogs(Barcode, UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_loots_scan_logs_box_updated
ON LootsScanLogs(Box_Id, UpdatedAt DESC);

CREATE INDEX IF NOT EXISTS idx_loots_scan_logs_updated
ON LootsScanLogs(UpdatedAt DESC);
";
            cmd.ExecuteNonQuery();
