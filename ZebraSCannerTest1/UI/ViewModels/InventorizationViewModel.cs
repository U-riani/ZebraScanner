...
public partial class InventorizationViewModel : ObservableObject, IDisposable
{
    ...
    private void RegisterProductUpdatedHandler()
    {
        ...
        {
            if (msg.Mode.HasValue && msg.Mode.Value != InventoryMode.Standard)
                return;
    ...
    private void ApplyProductUpdateToSlots(Product product)
    {
        var rows = Slots
            ...
            .Select(slot => (slot.Barcode, slot.ScannedQuantity, slot.InitialQuantity))
            .ToList();

        rows.Insert(0, (product.Barcode, product.ScannedQuantity, product.InitialQuantity));

        for (var i = 0; i < Slots.Count; i++)
        {
            if (i < rows.Count)
                Slots[i].Set(rows[i].Barcode, rows[i].ScannedQuantity, rows[i].InitialQuantity);
            else
